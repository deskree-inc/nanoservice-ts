			...
			this.app.use(
				...
				async (req: Request, res: Response): Promise<void> => {
					...
					try {
						...
						await this.configuration.init(blueprintNameInPath);

						let ctx: BlueprintContext = this.createContext(
							undefined,
							blueprintNameInPath || req.params.blueprint,
							id,
						);
						req.params = handleDynamicRoute(
							this.configuration.trigger.http.path,
							req,
						);

						ctx.logger.log(
							`Blueprint name: "${this.configuration.name}", version: "${this.configuration.version}"`,
						);
						...
						if (!validateRoute(path, req.path))
						...
						ctx.logger.log(
							`Workflow Runner completed in ${(end - start).toFixed(2)}ms`,
						);

						if (
							ctx.response.contentType === undefined ||
