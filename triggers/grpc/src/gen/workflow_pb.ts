// @generated by protoc-gen-es v2.2.3 with parameter "target=ts"
// @generated from file workflow.proto (package nanoservice.workflow.v1, syntax proto3)
/* eslint-disable */

import type { Message } from "@bufbuild/protobuf";
import type { GenEnum, GenFile, GenMessage, GenService } from "@bufbuild/protobuf/codegenv1";
import { enumDesc, fileDesc, messageDesc, serviceDesc } from "@bufbuild/protobuf/codegenv1";

/**
 * Describes the file workflow.proto.
 */
export const file_workflow: GenFile =
	/*@__PURE__*/
	fileDesc(
		"",
	);

/**
 * @generated from message nanoservice.workflow.v1.WorkflowRequest
 */
export type WorkflowRequest = Message<"nanoservice.workflow.v1.WorkflowRequest"> & {
	/**
	 * @generated from field: string Name = 1;
	 */
	Name: string;

	/**
	 * @generated from field: string Message = 2;
	 */
	Message: string;

	/**
	 * @generated from field: string Encoding = 3;
	 */
	Encoding: string;

	/**
	 * @generated from field: string Type = 4;
	 */
	Type: string;
};

/**
 * Describes the message nanoservice.workflow.v1.WorkflowRequest.
 * Use `create(WorkflowRequestSchema)` to create a new message.
 */
export const WorkflowRequestSchema: GenMessage<WorkflowRequest> = /*@__PURE__*/ messageDesc(file_workflow, 0);

/**
 * @generated from message nanoservice.workflow.v1.WorkflowResponse
 */
export type WorkflowResponse = Message<"nanoservice.workflow.v1.WorkflowResponse"> & {
	/**
	 * @generated from field: string Message = 1;
	 */
	Message: string;

	/**
	 * @generated from field: string Encoding = 2;
	 */
	Encoding: string;

	/**
	 * @generated from field: string Type = 3;
	 */
	Type: string;
};

/**
 * Describes the message nanoservice.workflow.v1.WorkflowResponse.
 * Use `create(WorkflowResponseSchema)` to create a new message.
 */
export const WorkflowResponseSchema: GenMessage<WorkflowResponse> = /*@__PURE__*/ messageDesc(file_workflow, 1);

/**
 * @generated from enum nanoservice.workflow.v1.MessageEncoding
 */
export enum MessageEncoding {
	/**
	 * @generated from enum value: BASE64 = 0;
	 */
	BASE64 = 0,

	/**
	 * @generated from enum value: STRING = 1;
	 */
	STRING = 1,
}

/**
 * Describes the enum nanoservice.workflow.v1.MessageEncoding.
 */
export const MessageEncodingSchema: GenEnum<MessageEncoding> = /*@__PURE__*/ enumDesc(file_workflow, 0);

/**
 * @generated from enum nanoservice.workflow.v1.MessageType
 */
export enum MessageType {
	/**
	 * @generated from enum value: TEXT = 0;
	 */
	TEXT = 0,

	/**
	 * @generated from enum value: JSON = 1;
	 */
	JSON = 1,

	/**
	 * @generated from enum value: XML = 2;
	 */
	XML = 2,

	/**
	 * @generated from enum value: HTML = 3;
	 */
	HTML = 3,

	/**
	 * @generated from enum value: BINARY = 4;
	 */
	BINARY = 4,
}

/**
 * Describes the enum nanoservice.workflow.v1.MessageType.
 */
export const MessageTypeSchema: GenEnum<MessageType> = /*@__PURE__*/ enumDesc(file_workflow, 1);

/**
 * @generated from service nanoservice.workflow.v1.WorkflowService
 */
export const WorkflowService: GenService<{
	/**
	 * @generated from rpc nanoservice.workflow.v1.WorkflowService.ExecuteWorkflow
	 */
	executeWorkflow: {
		methodKind: "unary";
		input: typeof WorkflowRequestSchema;
		output: typeof WorkflowResponseSchema;
	};
}> = /*@__PURE__*/ serviceDesc(file_workflow, 0);
