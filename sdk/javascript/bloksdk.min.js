class BlokSDK {
	createHttpClient(host, token, debug = false) {
		...
		return new BlokSDKClient({ host: _host, token: _token, debug: debug });
	}
}
class BlokSDKClient {
	constructor(config) {
	...
	async call(runtime, nodeName, inputs) {
		...
		const workflow = {
			...
			version: "1.0.0",
			trigger: { http: { method: "POST", path: "*", accept: "application/json" } },
		...
		const message = { Name: nodeName, Message: base64Workflow, Encoding: "BASE64", Type: "JSON" };
		if (this.debug) console.log("Request Message", JSON.stringify(message));
		try {
			const sdk_response = { success: false, data: null, rawData: null, errors: null, contentType: null, status: null };
			const fetchUrl = `${this.host}/${message.Name}`;
			if (this.debug) console.log("Fetch URL", fetchUrl);
			const response = await fetch(fetchUrl, { method: "POST", headers: requestHeader, body: JSON.stringify(message) });
			...
			if (!response.ok) {
				...
				if (contentType?.includes("application/json")) {
					...
					sdk_response.errors = Array.isArray(responseJson)
						? responseJson.map((error) => ({ status: response.status, ...error }))
						: [{ status: response.status, ...responseJson }];
				} else sdk_response.errors = { status: response.status, message: response.statusText };
				return sdk_response;
			}
			...
			return {
				...
				rawData: null,
				contentType: null,
				errors: [{ status: 500, message: error.message || "Unexpected error" }],
			};
		}
	}
}
