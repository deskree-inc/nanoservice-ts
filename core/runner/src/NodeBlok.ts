...
import type { INodeBlokResponse } from "./NodeBlokResponse";
import type RunnerNode from "./RunnerNode";
import { type ConfigContext, type Context, Metrics, NodeBase, type ResponseContext } from "./shared";
import type VarsContext from "./shared/types/VarsContext";
import type Condition from "./types/Condition";
import type JsonLikeObject from "./types/JsonLikeObject";
import type ParamsDictionary from "./types/ParamsDictionary";

export default abstract class NodeBlok<T> extends NodeBase {
	...
	public async run(ctx: Context): Promise<ResponseContext> {
		...
		if (this.set_var) {
			...
			(response.data as unknown as NodeBlok<T>).contentType = this.contentType;
		}
	...
	): Promise<INodeBlokResponse | NodeBlok<T>[]>;
