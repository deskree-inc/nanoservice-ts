...
<Tip> In nanoservice-ts, **CTX** (short for **Context**) enables dynamic data flow within a workflow by providing shared access to request data, node outputs, and workflow variables. This example demonstrates how CTX is used to control workflow behavior based on incoming request parameters.   </Tip>
### Global Function