...
Triggers are the entry points to your Nanoservice-ts [Workflows](./workflows.mdx). They are specific events or conditions that, when they occur, initiate the execution of an associated workflow. Without a trigger, a workflow is just a definition; triggers bring them to life by starting the process.
...
```json
{
  "name": "getUserProfile",
  "trigger": {
    "type": "Http", // Specifies the trigger type
    "config": {      // Configuration specific to the HTTP trigger
      "path": "/users/{userId}", // The URL path, with a path parameter
      "method": "GET"            // The HTTP method (GET, POST, PUT, etc.)
    }
  },
  "nodes": [
...
### Example: Scheduled (Cron) Trigger Configuration

```json
{
  "name": "dailyReportWorkflow",
  "trigger": {
    "type": "Schedule",
    "config": {
      "cron": "0 2 * * *" // Cron expression for 2:00 AM every day
    }
  },
  "nodes": [
    // ... nodes for generating the daily report ...
  ]
}
```
*   `type: "Schedule"` declares it as a scheduled trigger.
*   `config.cron: "0 2 * * *"` uses a standard cron expression.

### Example: gRPC Trigger Configuration (Conceptual)

```json
{
  "name": "processPaymentGrpc",
  "trigger": {
    "type": "gRPC",
    "config": {
      "service": "PaymentService",
      "method": "ProcessPayment",
      "protoPath": "protos/payment.proto" // Path to the .proto definition file
    }
  },
  "nodes": [
    // ... nodes for processing the payment ...
  ]
}
```
*   `type: "gRPC"` declares it as a gRPC trigger.
*   `config.service` and `config.method` specify the gRPC service and method to listen for.
*   `config.protoPath` points to the Protocol Buffer definition file that defines the service and message types.
...
*   Request headers (e.g., `context.trigger.headers.authorization`)
*   Query parameters (e.g., `context.trigger.query.searchTerm`)
*   Path parameters (e.g., `context.trigger.params.userId`)
*   Request body (e.g., `context.trigger.body.payload`)
