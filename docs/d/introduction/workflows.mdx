...
For more complex workflows or when you prefer type safety, Nanoservice-TS also supports defining workflows using TypeScript. To create a TypeScript workflow:

1. Create a new `.ts` file in the `src/workflows/` directory (e.g., `src/workflows/my-workflow.ts`).
2. Use the `@nanoservice-ts/helper` package to define your workflow structure.

#### Example 1: Simple TypeScript Workflow

```typescript
// src/workflows/countries-helper.ts
...
This TypeScript workflow is functionally equivalent to the JSON example above but offers the benefits of TypeScript's type checking and IDE support.

#### Example 2: Conditional Logic in TypeScript Workflow

```typescript
// src/workflows/empty.ts
...
This example demonstrates how to use conditional logic in a TypeScript workflow, using the `AddIf` and `AddElse` helpers to create branching paths based on request parameters.
