...
*   **`nodes`**: An array of Node instances that make up the workflow. Each node instance specifies:
    *   `nodes`: The type or name of the Node to execute (e.g., `Http.Request`, `Utils.Logger`, or a custom node like `my-custom-node`).
    *   `inputs`: How data is mapped to the input schema of the Node. This can be static values, data from the trigger, or output from previous nodes.
