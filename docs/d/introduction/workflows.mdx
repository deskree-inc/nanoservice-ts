...
The easiest way to create a new workflow is using the `blokctl` CLI:
...
3. It has one step that uses the built-in `@blok-ts/api-call` node to fetch country data from an external API.
...
1. **`@blok-ts/api-call`**: Makes HTTP requests to external APIs and services.
2. **`@blok-ts/if-else`**: Provides conditional branching in workflows based on specified conditions.

You can use these built-in nodes directly in your workflow definitions by referencing their names (e.g., `"@blok-ts/api-call"`) in the `node` property of a step.
